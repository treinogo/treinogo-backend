...
async function seed() {
    try {
        ...
        const athleteProfiles = await prisma_1.prisma.athleteProfile.findMany({
        ...
        for (const planData of plans) {
            ...
            for (const weekData of planData.weeks) {
                await prisma_1.prisma.weeklyProgramming.create({
                    data: {
                        planId: plan.id,
                        week: weekData.week,
        ...
        for (let i = 0; i < athleteProfiles.length; i++) {
            ...
            for (let day = 30; day >= 0; day -= 3) {
                ...
                await prisma_1.prisma.training.create({
                    data: {
                        date: trainingDate,
                        type: ['CONTINUOUS_RUN', 'INTERVAL_TRAINING', 'LONG_RUN', 'FARTLEK'][Math.floor(Math.random() * 4)],
            ...
            for (let day = 1; day <= 7; day++) {
                ...
                await prisma_1.prisma.training.create({
                    data: {
                        date: trainingDate,
                        type: ['CONTINUOUS_RUN', 'INTERVAL_TRAINING', 'RECOVERY_RUN'][Math.floor(Math.random() * 3)],
        ...
        for (const challengeData of challenges) {
            ...
            for (const athlete of athleteProfiles.slice(0, 3)) {
                await prisma_1.prisma.challengeParticipant.create({
                    data: {
        ...
        for (const raceData of races) {
            await prisma_1.prisma.race.create({
                data: {
                    ...raceData,
        ...
        for (let i = 0; i < athleteProfiles.length; i++) {
            ...
            for (let t = 0; t < 2; t++) {
                ...
                await prisma_1.prisma.physicalTest.create({
                    data: {
                        athleteId: athlete.id,
                        testType,
                        pace,
