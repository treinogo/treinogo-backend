...
router.get('/athlete/:athleteId', auth_1.authenticate, (0, auth_1.authorizeRole)(['COACH', 'ATHLETE']), async (req, res) => {
    try {
        const userId = req.userId;
...
router.get('/:id', auth_1.authenticate, (0, auth_1.authorizeRole)(['COACH', 'ATHLETE']), async (req, res) => {
    try {
        const userId = req.userId;
        const { id } = req.params;
...
router.get('/stats/:athleteId', auth_1.authenticate, (0, auth_1.authorizeRole)(['COACH', 'ATHLETE']), async (req, res) => {
    try {
        const userId = req.userId;
        const { athleteId } = req.params;
        let hasAccess = false;
